...
def test_train_stem_option(client):
    ...
    df = pd.DataFrame({'uid': N_resp*[uid], 'free_response': responses, 'valid_label': valid})
    df_json = df.to_json()

    # Call the train route, get response
    # Grab both the parsed out dataframe and the coefficients (+ intercept)
    r = client.get("/train", query_string=urlencode(FEATURE_SET_1), json={"response_df": df_json})
    out = r.json
...
def test_train_domain_innovation(client):
    ...
    weights = OrderedDict(
        {
            'domain': 1, 'innovation': 2, 'bad': -2, 'common': 0
        }
    )
    vocab_dict = OrderedDict(
        {
    ...
    # Call the train route, get response
    # Grab both the parsed out dataframe and the coefficients (+ intercept)
    r = client.get("/train", query_string=urlencode(FEATURE_SET_2), json={"response_df": df_json})
    out = r.json
    output_df = pd.read_json(out['output_df'])
