...
df_analysis_mean = df_analysis_results[df_analysis_results["measure"] == "mean"].drop(
    "measure", axis=1
)
df_analysis_mean = df_analysis_mean.rename(
    columns={"accuracy_delta": "accuracy_delta__mean", "time_delta": "time_delta__mean"}
)
df_analysis_std = df_analysis_results[df_analysis_results["measure"] == "std"].drop(
    "measure", axis=1
)
df_analysis_std["accuracy_delta"] = df_analysis_std["accuracy_delta"] / np.sqrt(
    2 ** len(ctemp)
)
...
df1 = (
    df_final[df_final["metric"] == "mean"]
    .drop("metric", axis=1)
    .rename(columns={"value": "mean"})
)
df2 = (
    df_final[df_final["metric"] == "std"]
    .drop("metric", axis=1)
