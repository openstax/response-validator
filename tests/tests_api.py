...
# Users params -- where to get the response data and how much of it to
# sample for testing
...
# Simple helper function to process the result of the api call into
# something nice for a pandas dataframe
...
def do_api_call(response, uid, stops, nums, spell, nonwords, use_uid):
    ...
    params = {'response': response, 'uid': uid, 'remove_stopwords': stops,
              'tag_numeric': nums, 'spelling_correction': spell, 'remove_nonwords': nonwords}
    r = requests.get(base_url, params=params)
    D = r.json()
    stuff_i_want = [D['bad_word_count'], D['domain_word_count'],
                    D['innovation_word_count'], D['common_word_count']]
...
# Does not use stemming or common filtering (the latter just gets caught
# in garbage detection so meh)
counter = 1
for stops, nums, spell, nonwords, use_uid in product([True, False], repeat=5):
    ...
    # Compute the actual features and do the timing computation (normalized
    # per response)
    now = time.time()
    dft['vals'] = dft.apply(lambda x: do_api_call(
        x.free_response, x.uid, stops, nums, spell, nonwords, use_uid), axis=1)
    elapsed_time = (time.time() - now) / n_samp

    # Now, separate out the values, do classification stuff, and write result
...
cols = ['stopword_removal', 'numerical_tagging',
        'remove_nonwords', 'spelling_correction', 'use_domain_vocab']
...
for col in cols:
    ...
    df_true = df_analysis[df_analysis[col] == 'True'][
        ['key', col, 'accuracy', 'average_time']]
    df_false = df_analysis[df_analysis[col] == 'False'][
        ['key', col, 'accuracy', 'average_time']]
    ...
    temp = dft[['accuracy_delta', 'time_delta']].agg(
        {'mean', 'std'}).reset_index().rename(columns={'index': 'measure'})
    temp['variable'] = col
    df_analysis_results = df_analysis_results.append(temp)
df_analysis_mean = df_analysis_results[
    df_analysis_results['measure'] == 'mean'].drop('measure', axis=1)
df_analysis_mean = df_analysis_mean.rename(
    columns={'accuracy_delta': 'accuracy_delta__mean', 'time_delta': 'time_delta__mean'})
df_analysis_std = df_analysis_results[
    df_analysis_results['measure'] == 'std'].drop('measure', axis=1)
df_analysis_std['accuracy_delta'] = df_analysis_std[
    'accuracy_delta'] / np.sqrt(2**len(ctemp))
df_analysis_std['time_delta'] = df_analysis_std[
    'time_delta'] / np.sqrt(2**len(ctemp))
df_analysis_std = df_analysis_std.rename(
    columns={'accuracy_delta': 'accuracy_delta__std', 'time_delta': 'time_delta__std'})
...
df1 = df_final[df_final['metric'] == 'mean'].drop(
    'metric', axis=1).rename(columns={'value': 'mean'})
df2 = df_final[df_final['metric'] == 'std'].drop(
    'metric', axis=1).rename(columns={'value': 'std'})
