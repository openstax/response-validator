...
def do_api_time_call(response, uid, stops, nums, spell, nonwords, use_uid):
	...
	return ",".join([computation_time, validity_label])
...
now = time.time()
dft['time_valid'] = dft.apply(lambda x: do_api_time_call(x.free_response, x.uid, stops, nums, spell, nonwords, use_uid), axis=1)
elapsed_time_total = (time.time()-now)
elapsed_time = elapsed_time_total / n_samp

dft['computation_time'] = dft['time_valid'].apply(lambda x: x.split(',')[0]).astype(float)
dft['valid_prediction'] = dft['time_valid'].apply(lambda x: x.split(',')[1])
dft['valid_prediction'] = dft['valid_prediction'].map({'True': 'Valid', 'False': 'Invalid'})
