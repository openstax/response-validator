...
class EcosystemImporter(object):
	...
	def get_book_content(self, archive_url, book_cnx_id):
		...
		content = [self.get_page_content(book_cnx_id, page_id, archive_url) for page_id in id_list]
		...
		df_innovation['cvuid'] = df_innovation.apply(lambda x: x.book_id + ':' + x.page_id, axis=1)
		df_innovation = df_innovation[['cvuid', 'innovation_words']]
		all_vocab = df_innovation.groupby('cvuid')['innovation_words'].agg('sum').values[0]
	...
	def get_question_content(self, question_uid_list):
		...
		N_chunk = 100 # Limit of the API server on how many exercises we can get at a time
		question_list_chunks = [question_uid_list[x:x+N_chunk] for x in range(0, len(question_uid_list), N_chunk)]
		item_list = []
		for sublist in question_list_chunks:
			question_list_str = ",".join(sublist)
			question_json = requests.get(self.base_exercise_url.format(question_list_str))
		...
		for item in item_list:
			...
			for question in item['questions']:
				...
				answer_text = " ".join([self.format_cnxml(answer['content_html']) for answer in question['answers']])
				uid_list.append(uid)
				stem_list.append(stem_text)
				answer_list.append(answer_text)
		question_df = pd.DataFrame({'uid': uid_list, 'stem_text': stem_list, 'answer_text': answer_list})

		return question_df

	def parse_content(self, book_id, question_uid_list, archive_url='https://archive.cnx.org'):
