...
# ['uid', 'text', 'stem_text', 'option_text', 'contains_number', 'module_id',
# 'CNX Book Name', 'CNX Chapter Number', 'CNX Section Number']
...
path_to_export = (
    "/Users/drew/Box Sync/Research/External/NSF Highlight Share/2018 Data Complete/"
)
...
df["uid"] = df.apply(
    lambda x: str(x["Exercise Number"]) + "@" + str(x["Exercise Version"]), axis=1
)
...
df["text"] = df[["stem_text", "option_text"]].apply(
...
sp = StaxStringProc(corpora_list=["../corpora/all_plaintext.txt", "../corpora/big.txt"])
...
df_tutor = df_tutor.merge(
    df_book[
        ["CNX HTML URL", "CNX Book Name", "CNX Chapter Number", "CNX Section Number"]
    ],
    how="left",
)
df_tutor["module_id"] = df_tutor["CNX HTML URL"].apply(lambda x: x.split("/")[-1])
df_tutor = df_tutor[
    ["uid", "CNX Book Name", "CNX Chapter Number", "CNX Section Number", "module_id"]
]
