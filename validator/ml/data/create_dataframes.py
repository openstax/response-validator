...
df_bio_summary = df_bio_summary[
    ["book_name", "id", "module_id", "text", "tag"]
].drop_duplicates()
...
df_book_export["cnxmod"] = df_book_export["CNX HTML URL"].apply(
    lambda x: x.split("/")[-1].split("@")[0]
)
df_book_export = df_book_export.drop_duplicates(subset=["cnxmod"])
df_book_export = df_book_export[
    ["CNX Book Name", "CNX Chapter Number", "CNX Section Number", "cnxmod"]
].drop_duplicates()
...
df_final = df_final.sort_values(
    by=["CNX Chapter Number", "CNX Section Number"], ascending=True
)
df_grouped = (
    df_final.groupby(
        ["book_name", "CNX Chapter Number", "CNX Section Number", "module_id"]
    )["text"]
...
question_df = response_df.drop_duplicates(subset=["uid"])[
    ["subject_name", "uid", "chapter_id", "section_id"]
]
question_df = question_df.merge(
    df_grouped[["module_id", "CNX Chapter Number", "CNX Section Number"]],
    left_on=["chapter_id", "section_id"],
    right_on=["CNX Chapter Number", "CNX Section Number"],
    how="left",
)
question_df = question_df[
    ["subject_name", "uid", "chapter_id", "section_id", "module_id"]
