# This script creates a SymSpell spelling dictionary for use with the parser It
# takes in a combination of a previously existant symspell dictionary and
# external files for training
# The code flow is as follows:

# 1) Load up the original symspell dictionary
# 2) Create dictionaries for all of the external files
#    (textbooks, questions banks, etc) and append in one pd frame
# 3) Adjust the counts in the original dataframe to be comparable
#    to the external stuff (keep the prior favorable)
...
# symspell_dictionary is a symspell ready file (plaintext, term is 0th col,
# count is 1st col) external_files are each plaintext -- counts will be
# computed on-the-fly
...
df_final.to_csv(output_dictionary_name, header=False, index=None, sep=" ")
